...
toFrame (xdim, ydim) (x', y', mode, drawFrame) =
  ListFrame $
    map
      ( \y ->
          map
...
toFrameList (xdim, ydim) pixels (x, y, cursorMode, frameMode) = case frameMode of
...
eventTest events (x, y, mode, frameMode) = ((toFrameList dim helloTextPixel state'), state')
                where
                  state' = (x, y, mode, frameMode)
...
initState = (0, 0, 1, 0) -- For Info see type above.
